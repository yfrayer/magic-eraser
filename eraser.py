...
subprocess.run(['ffmpeg', '-i', videoin, '-framerate', framerate, filein]) 
...
if os.path.exists("video/audioout.aac"):
    audioin = "video/audioout.aac"
    subprocess.run(['ffmpeg', '-i', filein, '-i', audioin, '-c:a', 'libvorbis', '-framerate', framerate, '-c:v', 'libx264', videoout])
else:
    subprocess.run(['ffmpeg', '-i', filein, '-framerate', framerate, '-c:v', 'libx264', videoout])
...
subprocess.run(['ffmpeg', '-i', filein, '-framerate', framerate, '-c:v', 'libx264', videomask])
...
if os.path.exists("video/audioout.aac"):
    ...
    subprocess.run(['ffmpeg', '-i', videoin, '-i', videoresize, '-i', videomask, '-i', videoout, '-filter_complex', '[0:v][1:v]hstack[t];[2:v][3:v]hstack[b];[t][b]vstack[v]', '-map', '[v]', 'video/fourvideo.mp4'])
