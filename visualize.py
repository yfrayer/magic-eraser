...
def apply_mask(image, mask, color, flag, alpha=1):
    """Apply the given mask to the image.
       flag[0] == 1 means mask is not selected
       flag[1] == 1 means this is the first mask
       flag[2] == 1 means image does not contain any selections
    """
    for c in range(3):
        if flag[2] == 1 and flag[1] == 0:
           print("all masks are empty and not first")
           continue
        if flag[1] == 1:
            if flag[0] == 1:
                print("mask is not selected and is first")
                image[:, :, c] = np.where(all,
                                      0,
                                      image[:, :, c])
            else:
                print("mask is selected and first")
        ...
        if flag[0] == 0:
            ...
            print("mask is not selected")
    return image


def display_instances(selection, empty, image, boxes, masks, class_ids, 
                      class_names, scores=None, title="", figsize=(16, 16), 
                      ax=None):
    """
    ...
    for i in range(N):
        ...
        ax.text(x1, y1 + 8, caption,
                color='none', size=11, backgroundcolor="none")
        
        flag = [0, 0, 0]
        # Whether all masks are empty
        if empty == True:
                flag[2] = 1
                flag[0] = 1
        # Whether this mask is empty
                ...
                flag[0] = 1
        # Whether it's the first mask
        if i == 0:
                flag[1] = 1

        # Mask
        mask = masks[:, :, i]
        masked_image = apply_mask(masked_image, mask, color, flag)
        if flag[0] == 1:
                continue
...
def draw_rois(image, rois, refined_rois, mask, class_ids, class_names, limit=10):
    ...
    for i, id in enumerate(ids):
        ...
        if class_id:
            ...
            masked_image = apply_mask(masked_image, m, color, flag)
