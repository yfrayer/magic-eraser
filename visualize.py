...
def display_instances(objects, image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
    ...
    for i in range(N):
        ...
        # Label
        unique_id = objects[0][i]
        class_id = class_ids[i]
        score = scores[i] if scores is not None else None
        label = class_names[class_id]
        label2 = "{} {}".format(unique_id, label)
        x = random.randint(x1, (x1 + x2) // 2)
        caption = "{} {} {:.3f}".format(unique_id, label, score) if score else label2
