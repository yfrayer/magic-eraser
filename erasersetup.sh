...
# This setup will create a MagicEraser directory
sudo apt update || exit 1
sudo apt install -y python3-dev python3-pip || exit 1
sudo apt install python-pip || exit 1
...
mkdir MagicEraser || exit 1
cd MagicEraser || exit 1
wget https://raw.githubusercontent.com/yfrayer/magic-eraser/master/eraser.py || exit 1
...
cd .. || exit 1
cd Mask_RCNN || exit 1
rm visualize.py || exit 1
wget https://raw.githubusercontent.com/yfrayer/magic-eraser/master/visualize.py || exit 1
wget https://raw.githubusercontent.com/yfrayer/magic-eraser/master/visualize2.py || exit 1
wget https://raw.githubusercontent.com/yfrayer/magic-eraser/master/runmask.py || exit 1
wget https://github.com/matterport/Mask_RCNN/releases/download/v2.0/mask_rcnn_coco.h5 || exit 1
pip install 'keras==2.1.6' || exit 1
cd .. || exit 1
mkdir video || exit 1
cd video || exit 1
mkdir mask orig output overlay overlayvideo resize trim || exit 1
wget https://raw.githubusercontent.com/yfrayer/magic-eraser/master/play.py || exit 1
